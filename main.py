...
class YT_downloader:
    ...
    def caption(self,url: str, lan: str, filename: str = '', out_path: str = '') -> None:
        try:
            ...
            self.message += f'Finish --  {target.title} -- caption\n'
            self.display.update()
    ...
    def vdo(self, url: str, resolution: str, fps: int, filename: str = '', out_path: str = ''):
        try:
            ...
            logging.info(f'downloading {target.title} ({url}) -- video')
            if out_path == '':
            ...
            logging.info('Finish -- video')
            self.message += f'Finish --  {target.title} -- video\n'
            self.display.update()
    ...
    def sound(self, url: str, filename: str = '', out_path: str = ''):
        try:
            ...
            self.message += f'Finish --  {target.title} -- sound\n'
            self.display.update()
    ...
    def merge(self, vdo_path: str, audio_path: str, filename: str, out_path: str) -> None:
        try:
            ...
            logging.info(f'Finish -- download {filename} -- merge')
            self.message = f'Finish --  {filename} -- sound'
            self.display.update()
    ...
    def gui(self):
        ...
        def res_value_assign(event):
            try:
                ...
                logging.info('no resolution value')
        ...
        def caption_value_assign(event):
            try:
                ...
                logging.info('no caption language')
        ...
        try:
            ...
            type_vdo = tkinter.Radiobutton(input_type, text='video', variable=v, value=1)
            type_audio = tkinter.Radiobutton(input_type, text='audio', variable=v, value=2)
            type_l.pack()
            ...
            self.display = tkinter.Label(root, text=f' {self.message}')
            run.pack(side=tkinter.LEFT)

            self.display.pack(side=tkinter.BOTTOM)
